...
|---|---|---|---|
| `GET` | `/api/v1/exhibits` | `GetExhibits` | Paginated (`page`, `pageSize`), filterable by `name` (case-insensitive `ILike` partial match), sortable (`sortBy` ∈ `name`/`startDate`/`endDate`, `sortDirection`). Defaults to ascending `startDate` when no sort is given. |
